...
sealed class AssemblyPool
{
    ...
    private static readonly string[] ignore = new[] { "EnumExtender", "PublicityStunt", "AutoUpdate", "LogFix", "BepInEx-Partiality-Wrapper", "PartialityWrapper" };
            ...
            if (ret.modAssemblies.TryGetValue(name, out ModAssembly conflicting)) {
                ...
                if (asm.Name.Version.Major != conflicting.AsmDef.Name.Version.Major) {
                    progressable.Message(
                        ...
                        message: $"Two assemblies named {name} are incompatible: {asm.Name.Version} from {rwmod.Header.Name}, and {conflicting.AsmDef.Name.Version} from {conflicting.Rwmod.Header.Name}."
                        );
                    return null;
                }
