...
namespace Realm.Gui
{
    public sealed class RaindbPanel : RectangularMenuObject, IHoverable, IListable
    {
        ...
        public RaindbPanel(RaindbMod raindbMod, MenuObject owner, Vector2 pos) : base(owner.menu, owner, pos, new(Width, Height))
        {
            ...
            if (!string.IsNullOrEmpty(raindbMod.Repo)) {
                subObjects.Add(dnldButton = new SymbolButton(menu, this, "Menu_Symbol_Clear_All", "", new(size.x - 29, size.y - 53)));
                downloaded = ProgramState.Current.Mods.AllRwmods.Any(rwmf => rwmf.RepositoryName == raindbMod.Repo);
            }
        ...
        public override void Singal(MenuObject sender, string message)
        {
            if (sender == linkButton) {
                Process.Start(new ProcessStartInfo(RaindbMod.ModHomepage) { UseShellExecute = true }).Dispose();
                ...
                performingJob = Job.Start(() => {
                    ...
                    Execution pr = Execution.Run(Extensions.MutatorPath, $"--download \"{RaindbMod.Author}/{RaindbMod.Repo}\"");
